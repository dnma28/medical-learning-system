...
For milestone #97, follow [the Source Map execution plan](SOURCE_MAP_EXECUTION_PLAN.md): one coordinator, at most one evidence writer and one independent reviewer, and one active book until the pilot passes promotion/readback. A reviewer may read a frozen packet while its author works only on explicitly assigned exceptions in a successor artifact; they never overwrite the same version. Existing workers save a safe checkpoint before another batch is scheduled. Increase concurrency only after two consecutive books pass the same process and review capacity is available. Agent names/models are suggestions, not authority: changing a model label does not make the author an independent reviewer.
...
4. Continue through the authorized dependent steps while inputs are available; a chapter boundary or progress report is not a reason to ask the user to type "continue". Persist a resumable checkpoint after each bounded batch and before a tool/context limit: changed paths or immutable draft version, source/scope fingerprints, exact checks/results, stable exception IDs, completed units and the exact next action. Resume only the unfinished or invalidated units. Unchanged blockers are not new work. A worker may create subissues but the coordinator approves dependencies and nonoverlapping scopes before parallel execution.
...
For milestone #97, the architecture/staging RPC exists, but the corpus still needs complete source evidence and independent QA for 16 logical books. Close the Costanzo #116 audit first, then the separately authorized one-book certification/promotion/readback, before opening another book for execution. Use the [execution plan](SOURCE_MAP_EXECUTION_PLAN.md) for current ordering, retry limits, checkpoint contents and Sol/Luna responsibilities. Dated issue snapshots are evidence, not live authority. Do not use MarkItDown to turn uncertain PDF locators into `verified`.
