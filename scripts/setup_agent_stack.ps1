...
if (-not $SkipSpecKitInit) {
    if (Test-Path ".specify") {
        ...
        Write-Host "Run 'specify integration status' to inspect the current integration."
    }
    else {
...
Write-Host "  1. Run: specify integration status"
Write-Host "  2. Run: oh setup"
Write-Host "  3. Configure the provider you want OpenHarness to use (Codex Subscription is supported)."
Write-Host "  4. Run: oh --dry-run"
Write-Host ""
